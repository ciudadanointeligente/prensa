...
app.controller('newsController', ["$scope", "$http", "$timeout", function ($scope, $http, $timeout){
  get_news = "//api.morph.io/ciudadanointeligente/prensa-ci/data.json?key=C317BJoPzKOOMj%2B83VbD&query=select%20*%20from%20%27data%27%20order%20by%20fecha%20desc&callback=JSON_CALLBACK"
  $http.jsonp(get_news)
      .then( function (response){
        ...
        for (i in $scope.allnews){
          ...
          tmp_date = $scope.allnews[i]['fecha'].split('/')
          unixtime = new Date(tmp_date[2]+'/'+tmp_date[1]+'/'+tmp_date[0])
          news.unixtime = unixtime.getTime()
